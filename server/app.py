...
# How many features one request may RETRIEVE. Measured cost is about 2.5 minutes
# each (16 decadal COGs out of ~6 GB rasters), so an uncapped request on a large
# upload runs for hours and dies at the first proxy timeout.
#
# The cap counts cache misses only. Cached features cost a disk read, so a rerun
# over a whole survey stays instant, and a large upload can be worked through in
# batches: each run adds to the cache and the next one has less to do.
MAX_UNCACHED = int(os.environ.get("WWP_MAX_UNCACHED", 4))
MINUTES_PER_RETRIEVAL = 2.5
...
def health() -> dict[str, Any]:
    return {
        ...
        "mirror_etwapor": MIRROR_ETWAPOR,
        "max_uncached_per_request": MAX_UNCACHED,
...
class EstimatePlan(BaseModel):
    """What a request would cost, without doing any of it."""
...
def _plan(req: EstimateRequest) -> EstimatePlan:
    cached = 0
    for f in req.features:
        validate_feature(f)
        ck = cache.result_key(f.kind, geojson_geometry(f)["coordinates"], f.sos, f.eos,
                              req.scheme_code, MIRROR_ETWAPOR)
        if cache.get_result(ck) is not None:
            cached += 1
    total = len(req.features)
    todo = total - cached
    ok = todo <= MAX_UNCACHED
    if todo == 0:
        msg = f"All {total} features are cached; the run returns immediately."
    elif ok:
        msg = (f"{todo} of {total} features need retrieval, about "
               f"{todo * MINUTES_PER_RETRIEVAL:.0f} minutes.")
    else:
        msg = (
            f"{todo} of {total} features need retrieval, which is about "
            f"{todo * MINUTES_PER_RETRIEVAL:.0f} minutes - over the limit of "
            f"{MAX_UNCACHED} per request. Remove some features and run again: each "
            f"run is cached, so the next one has less to do."
        )
    return EstimatePlan(total=total, cached=cached, to_retrieve=todo, limit=MAX_UNCACHED,
                        within_limit=ok, estimated_minutes=round(todo * MINUTES_PER_RETRIEVAL, 1),
                        message=msg)


@app.post("/api/estimate/plan", response_model=EstimatePlan)
def estimate_plan(req: EstimateRequest) -> EstimatePlan:
    """Dry run: how many features are cached, how long the rest would take.

    Lets the dashboard say what a run will cost before committing to it, and
    makes the limit legible instead of arbitrary.
    """
    return _plan(req)
...
def estimate(
    ...
    # Refuse a run that cannot finish, before spending minutes discovering it.
    plan = _plan(req)
    if not plan.within_limit:
        raise HTTPException(422, plan.message)
